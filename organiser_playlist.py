...
avec des séparateurs comme pour la section CANADA
"""
...
for f in Path('.').glob('*.m3u'):
    if 'fusion' not in f.name.lower() and not f.name.endswith('.py'):
        playlist_file = f
        break
...
while i < len(lines):
    ...
    # Ignorer les séparateurs de section existants
    if line.startswith('# ============================================'):
        i += 1
        continue
    
    # Ignorer les lignes de commentaire de section
